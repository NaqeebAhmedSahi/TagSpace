...
// pg-cursor is optional - only needed for streaming queries
let Cursor: any = null;
try {
  Cursor = require("pg-cursor");
} catch (e) {
  // pg-cursor not available - streaming queries won't work but basic queries will
  console.warn('pg-cursor not available, streaming queries disabled');
}
import log from 'electron-log';
import { HasPool } from './types'
const pgCursorLog = log.scope('database:postgresql:cursor');
const logger = () => pgCursorLog;
...
export class PsqlCursor extends BeeCursor {
  ...
  async start() {
    if (!Cursor) {
      throw new Error('pg-cursor is not available. Please install it: npm install pg-cursor');
    }
