...
// Optional dependencies - stub if not available
let DuckDBBlobValue: any = null;
let ObjectId: any = null;
let RecordId: any = null;
let StringRecordId: any = null;

const transcodersLog = log.scope('database:transcoders');
const logger = () => transcodersLog;
...
export const SurrealDBRecordTranscoder: Transcoder<RecordId | StringRecordId, string> = {
  serialize(value) {
    if (!(value instanceof RecordId) && !(value instanceof StringRecordId)) {
      logger().warn("SurrealDBRecordTranscoder: cannot serialize non-recordid value");
      return value;
    }
