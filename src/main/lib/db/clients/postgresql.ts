...
import { AlterPartitionsSpec, IndexColumn, TableKey } from '../dialects/models';
...
const pgLog = log.scope('database:postgresql');
const logger = () => pgLog;
...
export class PostgresClient extends BasicDatabaseClient<QueryResult> {
    ...
    if (this.server.config.redshiftOptions?.iamAuthenticationEnabled) {
      this.interval = setInterval(async () => {
        try {
          ...
          logger().info('Token refreshed successfully and connection pool updated.');
        } catch (err) {
          logger().error('Could not refresh token or update connection pool!', err);
        }
  ...
  protected async configDatabase(server: IDbConnectionServer, database: { database: string}) {
    ...
    const passwordValue = await refreshTokenIfNeeded(server.config?.redshiftOptions, server, server.config.port || 5432) || server.config.password;
    ...
    const config: PoolConfig = {
      ...
      connectionTimeoutMillis: 30000, // 30 seconds default
      idleTimeoutMillis: 30000, // 30 seconds default
    };

    // Only set password if it's provided (avoid undefined causing SASL errors)
    if (passwordValue) {
