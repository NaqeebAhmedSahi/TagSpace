...
export default function Root({ store, persistor }: RootType) {
  ...
  // Ensure `AuthGate` can access the UserContext by providing the provider one level above it.
  // For builds without Amplify wrap the AuthGate inside the provider so AuthGate can access the context.
  if (!AppConfig.isAmplify) {
    ...
    // When Amplify is enabled, just provide the UserContext around the appContent
    appContent = <UserContextProvider>{appContent}</UserContextProvider>;
  }
