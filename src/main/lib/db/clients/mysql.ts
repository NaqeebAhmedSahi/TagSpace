...
// Optional ed25519 auth plugin - only used for specific MySQL auth methods
let ed25519AuthPlugin: any = null;
try {
  ed25519AuthPlugin = require("@coresql/mysql2-auth-ed25519");
} catch (e) {
...
import { MySqlChangeBuilder } from "../change_builder/MysqlChangeBuilder";
import { AlterTableSpec, IndexColumn, TableKey } from "../dialects/models";
...
const mysqlLog = log.scope('database:mysql');
const logger = () => mysqlLog;
...
// Helper function to parse version string like "8.0.43" into major, minor, patch
...
function isVersionLessThanOrEqual(
  currentVersion: { major: number; minor: number; patch: number },
  compareVersion: { major: number; minor: number; patch: number }
): boolean {
  if (currentVersion.major < compareVersion.major) return true;
  if (currentVersion.major > compareVersion.major) return false;
  if (currentVersion.minor < compareVersion.minor) return true;
  if (currentVersion.minor > compareVersion.minor) return false;
  if (currentVersion.patch < compareVersion.patch) return true;
  if (currentVersion.patch > compareVersion.patch) return false;
  return true; // equal
}
...
async function configDatabase(
  ...
  if (!server || !server.config) {
    throw new Error('Invalid server configuration: server.config is undefined');
  }
  if (!database) {
    throw new Error('Invalid database configuration: database is undefined');
  }

  const config: mysql.PoolOptions = {
    authPlugins: ed25519AuthPlugin ? {
      'client_ed25519': ed25519AuthPlugin,
    } : {},
    host: server.config.host || 'localhost',
    port: server.config.port || 3306,
    user: server.config.user || undefined,
    ...
    connectTimeout: 30000, // 30 seconds default
  };
