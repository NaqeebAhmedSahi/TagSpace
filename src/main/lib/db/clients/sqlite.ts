...
import { IndexColumn, TableKey } from "../dialects/models";
import { SqliteData } from "../dialects/sqlite";
import { ChangeBuilderBase } from "../change_builder/ChangeBuilderBase";
import { SqliteChangeBuilder } from "../change_builder/SqliteChangeBuilder";
...
const sqliteLog = log.scope('database:sqlite');
const logger = () => sqliteLog;
...
export class SqliteClient extends BasicDatabaseClient<SqliteResult> {
  ...
  protected async rawExecuteQuery(q: string, options: any): Promise<SqliteResult | SqliteResult[]> {
    ...
    const runtimeExt = this.server?.config?.runtimeExtensions || [];
    console.log("Extensions: ", runtimeExt)
    if (runtimeExt && runtimeExt.length > 0) {
      for (const extension of runtimeExt) {
        try {
  ...
  protected createRawConnection(filename: string) {
    // Normalize filename and handle directory cases (e.g. './') which cause
    // better-sqlite3 to raise SQLITE_CANTOPEN. If the caller passed a
    // directory path, open a file named 'tagspaces.sqlite' inside it.
    let dbPath: string | undefined;
    try {
      dbPath = filename;

      // allow :memory: to pass through
      if (dbPath !== ':memory:') {
        // Resolve relative paths
        let resolved = path.resolve(dbPath);

        // If the resolved path exists and is a directory, use a default file inside it
        try {
          if (fs.existsSync(resolved) && fs.lstatSync(resolved).isDirectory()) {
            resolved = path.join(resolved, 'tagspaces.sqlite');
          } else if (dbPath === '.' || dbPath === './' || dbPath.endsWith(path.sep)) {
            // handle cases like './' or ending with path separator
            resolved = path.join(resolved, 'tagspaces.sqlite');
          }
        } catch (e) {
          // ignore lstat errors and fall back to original filename
        }
        ...
        try {
          ...
          console.error('Failed to create parent directory for sqlite file', parent, e);
        }
      ...
      // Provide detailed diagnostic info to the console to help debugging native binding / file path issues
      const isABIMismatch = err?.message?.includes('NODE_MODULE_VERSION') || err?.code === 'ERR_DLOPEN_FAILED';
      try {
        console.error('SqliteClient.createRawConnection error', {
          filename,
          resolvedFilename: (typeof dbPath !== 'undefined') ? dbPath : undefined,
          isABIMismatch,
          error: err && (err.stack || err.message) ? (err.stack || err.message) : String(err),
          pid: process.pid,
        ...
        if (isABIMismatch) {
          console.error('⚠️  SQLITE CONNECTION FAILED - Native Module ABI Mismatch');
          console.error('The better-sqlite3 native module is incompatible with the current Electron version.');
          console.error('To fix, run: npm rebuild better-sqlite3 --build-from-source');
          console.error('Or update to a compatible version: npm install better-sqlite3@latest');
        } else if (err && err.code === 'SQLITE_CANTOPEN') {
          console.error('⚠️  SQLITE_CANTOPEN: check that the provided database path is a file path and writable.');
          console.error('Provided path:', filename);
          console.error('Try using an explicit file path (e.g. ./data/tagspaces.sqlite) or :memory: for an in-memory DB.');
        }
      } catch (e) {
        // swallow
        console.error('SqliteClient.createRawConnection error (secondary):', err);
      }
      throw err;
    }
  }
