...
import { AlterPartitionsSpec, AlterTableSpec, CreateTableSpec, IndexAlterations, RelationAlterations, TableKey } from '../dialects/models';
...
// Create logger - electron-log scope returns a logger instance
...
export abstract class BasicDatabaseClient<RawResultType extends BaseQueryResult> implements IBasicDatabaseClient {
  ...
  async checkAllowReadOnly() {
    // Stub: Always allow read-only mode for now
    return true;
  }
  ...
  async connect(_signal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      if (this.server?.config?.ssh && !this.server.sshTunnel) {
        ...
        // SSH tunnel support not implemented yet
        ...
        logger().warn('SSH tunnel not yet supported');
      }
